#!/usr/bin/env python3
"""Rank predicted structures by a simple extension proxy."""
...
def find_confidence_json(struct_path: Path) -> Path | None:
    ...
    candidates = list(struct_path.parent.glob(f"*summary_confidence*sample_{sample_idx}.json"))
    if not candidates:
        return None
    return candidates[0]
...
def read_confidence(path: Path | None) -> Tuple[str, str]:
    ...
    try:
        ...
        return ("", "")
    # Protenix/Boltz variants may use different keys; handle common cases.
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Rank structures by extension proxy.")
    parser.add_argument("--pred-root", required=True, type=Path)
    parser.add_argument("--out", required=True, type=Path)
    args = parser.parse_args()

    if not args.pred_root.exists():
        raise FileNotFoundError(f"Prediction root not found: {args.pred_root}")
    ...
    for path in struct_files:
        ...
        conf_path = find_confidence_json(path)
        iptm, ptm = read_confidence(conf_path)
    ...
    print(f"[rank] top structure: {rows[0][1]} (span={rows[0][0]:.3f})")
    return 0
