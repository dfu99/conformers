...
### Conda OpenMM on RunPod: pin `cuda-version` to the driver's max CUDA, not the newest
- Command context: 2026-06-30, route-A Stage-1 env build on the A5000 pod. `mamba create ... openmm` pulled `cuda-version=13.3` (newest on conda-forge), but the pod's driver (580.126.09) supports only CUDA 13.0. OpenMM's CUDA context creation failed with `CUDA_ERROR_UNSUPPORTED_PTX_VERSION (222)` — the PTX was compiled for a CUDA newer than the driver allows (forward-incompatible).
- Symptom: env imports fine, `Platform.getPlatformByName("CUDA")` exists, but `mm.Context(..., CUDA)` throws UNSUPPORTED_PTX_VERSION. The failure is at context creation, not import.
- Root cause: conda-forge ships openmm builds per `cuda-version`; the solver defaults to the newest. A newer toolkit's PTX won't JIT on an older driver. `nvidia-smi` "CUDA Version: X.Y" is the driver's *max* supported CUDA — the env's `cuda-version` must be ≤ that.
- Action: when creating an OpenMM (or any CUDA) conda env on a RunPod/cluster GPU, FIRST read `nvidia-smi` "CUDA Version", then create with `mamba create ... "cuda-version=<=driver_cuda>" openmm` (12.6 is a safe universal floor for any recent driver). Verify with a 1-particle `mm.Context(..., CUDA)` smoke before building anything large.
- Practical implication: the smoke test (1-particle CUDA context) catches this in seconds; discovering it after a 470k-atom solvate + minimize wastes ~10 min. Always GPU-smoke the platform before the real system.

### `pkill -f <pattern>` matches the calling shell's OWN command line — kill by PID
- Command context: 2026-06-30. To kill an orphaned `mamba install -y -p /workspace/envs/route_a ...`, I ran (over ssh) `pkill -9 -f "mamba install -y -p /workspace/envs/route_a"` in a script that ALSO contained that literal string (in the pkill arg and a relaunch line). `pkill -f` matched my own ssh shell's command line and killed the session (ssh exit 255); the relaunch never ran.
- Symptom: ssh exits 255 right after a `pkill -f`; the intended follow-on commands silently don't execute.
...
### Check whether a CV/config file is real or a test/placeholder before building on it
- Command context: 2026-07-04, route-A Stage 2. I'd been reporting cv0/cv1/cv2 from `pipelines/route_a/examples/av_cvs_remapped.json` since Stage 1 as if they were the reaction coordinate. Building the extended state, cv0 (head-calf) barely moved (54.26→53.86) while the molecule clearly extended (Rg 39→66 Å). Reading the file's own header: `"_source": "Synthetic ... αIIb-numbered CV definitions for testing remap_cvs.py"`. They are placeholders, and cv0's residue range (A:442-605) is actually the thigh (stays with the head), so it can't measure leg extension.
- Symptom: a "reaction coordinate" that fails to move when the physics obviously changes. Months of runs quoted a CV that doesn't track the process it's named for.
- Root cause: a file created to test a *converter* (remap_cvs.py) got reused as if it were validated science input. Its docstring/`_source` said "synthetic/testing" but that was never re-read.
- Action: before using any CV / config / parameter file as a scientific input, read its provenance header (`_source`, comments, README) and sanity-check that it actually responds to the process. For a CV: confirm it changes monotonically between the two endpoints you care about (here, bent vs extended). If a metric doesn't separate the endpoints, it's the wrong metric.
- Practical implication: global shape metrics (Rg, long-axis extent, head-tip↔leg-tip distance) unambiguously captured integrin extension where the placeholder CVs did not. Route-A Stage 2 must define real extension CVs before the string method. Don't trust a file's *name*; verify its *behavior*.

### A one-shot rigid-body domain rotation makes an unminimizable clash — morph incrementally instead
- Command context: 2026-07-04, route-A Stage 2b/2c. Building the extended αVβ3 endpoint by a single 139° rigid rotation of the lower legs about the genu hinge (`stage2_build_extended.py`) produced a genu clash with potential energy ~1e13 kJ/mol. No direct energy minimize could descend it: a CPU worker held 715% for ~4 min without completing a single 100-iteration chunk, both with `constraints=HBonds` and with `constraints=None`. The one enormous clash gives the L-BFGS line search no foothold.
- Symptom: `minimizeEnergy` never returns / energy never drops from an astronomically high starting value after a hand-built large conformational change.
- Root cause: a big single-jump rigid rotation slams the moving domain into the fixed one all at once (localized but severe overlap at the hinge). Minimizers relax *local* strain well but cannot walk down a ~1e13 cliff.
- Action: build the large motion INCREMENTALLY — rotate a small step (~9°) about the hinge, energy-minimize, repeat (adaptive: stop when the moving domain reaches the target direction). Each small step introduces only a mild clash that minimizes trivially, so the energy stays sane (~1e4 kJ/mol) the whole way. Bonus: the per-step frames are a ready-made initial path for a string/NEB calculation. See `stage2c_morph_extend.py`.
- Practical implication: never expect a minimizer to fix a one-shot rigid-body construction of a large domain motion. Morph it in small relaxed steps.

### GB implicit solvent is slow and poorly threaded on the OpenMM CPU platform — use vacuum for clash relief
- Command context: 2026-07-04, route-A Stage 2c. A CPU implicit-solvent (GB-OBC2, amber14/ff14SB) minimize of the 22k-atom αVβ3 monomer would not finish even for the *unclashed* bent structure in 2 min; the process pinned only ~700% CPU (≈7 of 48 threads). The GB Born-radii evaluation (~O(N²) within cutoff) is the expensive part and does not parallelize well on the CPU platform.
- Symptom: an OpenMM CPU minimize/dynamics that is far slower than expected and uses only a fraction of the requested threads, even on a structure with no severe clash.
- Root cause: implicit-solvent GB force evaluation dominates cost (~1 s/eval here) and threads poorly on the CPU platform; hundreds of L-BFGS iterations therefore take many minutes.
- Action: for pure clash relief / geometry cleanup that doesn't need solvation accuracy, drop implicit solvent and minimize in **vacuum** (amber14 protein FF, `CutoffNonPeriodic` ~1.2 nm) — ~5–10× faster per iteration. The final/production relaxation can use explicit solvent on a GPU later. Reserve GB-on-CPU for small systems or when solvation energy actually matters.
- Practical implication: switching Stage 2c from GB to vacuum turned a stalled run into a 16-step morph that finished in minutes; the extended shape is held by the rigid-body geometry, not by solvent, so vacuum clash relief is scientifically fine for a seed.

### OpenMM/PDBFixer renumbers residues contiguously on write — match by rank, not by resid, when comparing structures
- Command context: 2026-07-06, route-A obj-076 (defining extension CVs). The bent reference `data/reference_pdbs/1jv2.pdb` keeps crystal numbering (chain A 1..956 with gaps, chain B 55..690); the extended `extended_state_b.pdb`, written by OpenMM after PDBFixer, is renumbered **contiguous** (A 1..927, B 1..539). A CV defined as "chain B residues 436-444" therefore selects DIFFERENT physical residues in the two files — silently, with no error.
- Symptom: a per-residue / per-domain CV gives nonsense or non-comparable values between an input crystal and its OpenMM-processed derivative, even though "the same residue ranges" were used.
- Root cause: PDBFixer preserves author numbering internally during processing, but the OpenMM `PDBFile` writer emits residues renumbered 1..N per chain. Same Cα set, same order, different labels.
- Action: when comparing two structures that share a residue set but not numbering, match Cα by **rank in sorted order** (i-th present Cα ↔ i-th present Cα), not by residue id. Assert equal CA counts per chain first. Key all CV definitions to ONE canonical numbering (here the original 1JV2) and translate the other file onto it. Cross-check with a numbering-independent metric (Rg): local bent Rg 39.23 matched the pod's recorded 39.14 → parser + mapping validated.
- Practical implication: the string-method port will compare CVs across the bent endpoint, the morph frames, and the extended endpoint — all potentially renumbered. Compute CVs from a rank-matched, single-canonical-numbering selection (see `pipelines/route_a/scripts/define_extension_cvs.py`), never from raw resid ranges across heterogeneous files.

### Local OpenMM GPU MD: base env has no CUDA platform — build an isolated CUDA env and pre-flight every system before a GPU window
- Command context: 2026-07-07/08, route-A snap-back test setup. The machine's default (anaconda base) OpenMM 8.5 exposed only `Reference` + `CPU` platforms — the plugins dir had OpenCL but NO CUDA plugin, and OpenCL failed to load — so the idle local RTX 3060 was unusable by it. Fix: `conda create -n snapback -c conda-forge openmm python=3.11` gives a CUDA-enabled build in an ISOLATED env (no risk to the shared base). Then the snap-back run uses `/home/dan/anaconda3/envs/snapback/bin/python` with `CUDA_VISIBLE_DEVICES=0`.
- Gotcha 1: `conda create ... openmm` does NOT pull in **pdbfixer** — install it separately (`pip install pdbfixer` into the env). A build-only pre-flight caught this before it could crash a live GPU window.
- Gotcha 2: **GB implicit solvent is hopeless for CPU DYNAMICS**, not just minimization (existing lesson covered minimization). At ~1 s/force-eval for a 22k-atom system, a 20 ns run (10M steps) would take months on CPU. Validate the machinery in VACUUM on CPU (fast: stepping + observables), and run the GB production on GPU. A vacuum smoke confirmed mutation/FF/observables; a build-only implicit pre-flight confirmed all systems parametrize.
- Action pattern for any multi-system MD: (a) isolate a CUDA-openmm+pdbfixer env; (b) `--build-only` every system (WT + each mutant) to confirm it parametrizes in the PRODUCTION force field and that observables read correctly, BEFORE requesting the GPU; (c) respect the local-GPU scheduler — do NOT run without a "GPU ACCESS GRANTED" message, and never contend with the oxDNA FFS jobs on the shared pod. See `snapback_md.py --build-only` and `run_snapback_suite.sh`.

### Route-A GPU runs go on the RunPod A5000, NOT the local GPU (PI directive 2026-07-08)
- Command context: after I set up an isolated local CUDA-OpenMM env to run the snap-back MD on the idle local RTX 3060, the PI said verbatim: "Do not use local GPU. Just wait for a GPU slot on the A5000." So route-A GPU work (snap-back MD, and later the string-method MD evolution) runs on the shared RunPod A5000, waiting for a slot to open — never on the local 3060.
- Codified: the local `run_snapback_suite.sh` is marked DEPRECATED/DO-NOT-USE in its header; the live path is `run_snapback_suite_pod.sh` (pod env `/workspace/envs/route_a`, `CUDA_VISIBLE_DEVICES=0`) launched by `wait_and_run_snapback.sh` (a conservative watcher that waits for oxDNA to be absent 30 consecutive minutes before taking the A5000, so it can never contend).
- Action: for any route-A GPU job, stage it on the pod and wait for an A5000 slot; do NOT reach for the local GPU even when it's idle and a scheduler grant is available. "Wait for a slot on the A5000" is the standing instruction. Never contend with the caDNAgentic oxDNA FFS jobs on that pod.

### Static force-field interaction energies are a CPU-instant companion to expensive dynamics — read them straight off the NonbondedForce (no Context, no platform, no GPU)
